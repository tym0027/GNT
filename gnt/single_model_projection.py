...
def __init__(self, device):
    self.device = device
'''
def projector_inbound(pixel_locations, h, w):
    """
    ...
    :return: mask, bool, [...]
    """
    return ((pixel_locations[..., 0] <= w - 1.0)
            & (pixel_locations[..., 0] >= 0)
            & (pixel_locations[..., 1] <= h - 1.0)
            & (pixel_locations[..., 1] >= 0))

def projector_normalize(pixel_locations, h, w):
    resize_factor = torch.tensor([w - 1.0, h - 1.0]).to(pixel_locations.device)[None, None, :]
    normalized_pixel_locations = (2 * pixel_locations / resize_factor - 1.0)  # [n_views, n_points, 2]
    return normalized_pixel_locations

def projector_compute_projections(xyz, train_cameras):
    """
    ...
    projections = train_intrinsics.bmm(torch.inverse(train_poses)).bmm(xyz_h.t()[None, ...].repeat(num_views, 1, 1))  # [n_views, 4, n_points]
...
def projector_compute_angle(xyz, query_camera, train_cameras):
    """
    :param xyz: [..., 3]
...
def projector_compute(xyz, query_camera, train_imgs, train_cameras, featmaps):
    """
    ...
    """
    print(train_imgs.shape, train_cameras.shape, query_camera.shape, featmaps.shape)
    assert ((train_imgs.shape[0] == 1) and (train_cameras.shape[0] == 1) and (query_camera.shape[0] == 1)), "only support batch_size=1 for now"
    ...
    pixel_locations, mask_in_front = projector_compute_projections(xyz, train_cameras)
    normalized_pixel_locations = projector_normalize(pixel_locations, h, w)  # [n_views, n_rays, n_samples, 2]
    ...
    # mask
    inbound = projector_inbound(pixel_locations, h, w)
    ray_diff = projector_compute_angle(xyz, query_camera, train_cameras)
